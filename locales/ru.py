...
oc_watermark = "{} Вотермарка сообщений"
...
# Ссылки
lnk_github = "🛠️ Создать своего бота FunPay"
...
access_granted_notification = "<b>🚨 ВНИМАНИЕ! 🚨\n\n\n</b>" * 3 + "\n\n\n🔐 \"<a href=\"tg://user?id={1}\"> {0} </a>\" " \
                                                                 "<b>(ID: {1}) только что получил доступ к ПУ Telegram! 🔓</b>"
...
✅ <i>FPC</i> может точно определить, кто именно написал, будь то ты, твой собеседник или 3е лицо (арбитраж).
❌ Из-за того что <i>FPC</i> получает полную историю чата для обнаружения новых сообщений, чат становится "прочитанным" (не загорается оранжевым).
...
❌ <i>FPC</i> не получает полную историю чата, вследствие чего видит только последнее сообщение, а не все. Если пользователь быстро напишет несколько сообщений, <i>FPC</i> увидит только последнее.
❌ <i>FPC</i> не может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
❌ <i>FPC</i> не может точно определить, кто именно написал: ты или собеседник. Если чат не прочитан, значит сообщение от собеседника, иначе - от тебя. Однако, если ты смотришь чат в момент получения сообщений, данная логика иногда может сбоить. Так же <i>FPC</i> не сможет определить, что в чат написало 3е лицо (арбитраж).

❗ Если нажать кнопку <code>Больше</code> в уведомлении о новом сообщении, <i>FPC</i> "прочитает" чат и покажет последние 15 сообщений, в том числе изображения. Так же <i>FPC</i> сможет определить, кто именно автор сообщений."""
...
❌ <i>FunPay Cardinal</i> еще <b><u>не инициализирован</u></b> и <b><u>никакие его функции не работают</u></b>.\n
...
👨‍💻 <b><i>Разработчик:</i></b> @woopertail, @sidor0912
...
<i>Обновления:</i> @fpc_updates
<i>Плагины:</i> @fpc_plugins
...
proxy_already_exists = "❌ Прокси <code>{}</code> уже добавлены."
proxy_added = "✅ Прокси <u>{}</u> добавлены успешно."
proxy_format = "❌ Прокси должны иметь формат <u>login:password@ip:port</u> или <u>ip:port</u>."
...
update_lasted = "✅ У тебя стоит последняя версия Funpay {}"
...
update_backup = "✅ Резервная копия конфигов, хранилища и плагинов <code>backup.zip</code>.\n\n" \
                "⚠️ НЕ ОТПРАВЛЯЙ этот архив НИКОМУ. Он содержит абсолютно ВСЁ содержимое и настройки бота (в том числе golden_key и товарные файлы)."
update_backup_error = "❌ Не удалось создать бэкап конфигов, хранилища и плагинов."
update_backup_send_error = "❌ Не удалось отправить резервную копию."
update_backup_not_found = "❌ Резервная копия не обнаружена."
update_downloaded = "✅ Загрузил обновление {} (пропущено {} шт.). Устанавливаю..."
update_download_error = "❌ Произошла ошибка при загрузке обновления."
update_done = "✅ Установил обновление! Перезапусти <i>FPC</i> с помощью команды /restart"
update_done_exe = "✅ Установил обновление! Новый <code>FPC.exe</code> находится в папке <code>update</code>. " \
                  "Выключи <i>FPC</i>, перенеси новый <code>FPC.exe</code> на место старого " \
...
act_change_golden_key = "Введите golden_key"
cookie_changed = "✅ golden_key успешно изменен{}.\n"
cookie_changed2 = "Перезапустите бота командой /restart."
cookie_incorrect_format = "❌ Неверный формат golden_key. Попробуй ещё раз."
cookie_error = "❌ Не удалось авторизоваться. Возможно, введен неверный golden_key?"
...
pl_new = "Отправь мне плагин.\n\n<b>⚠️ ВНИМАНИЕ! Загрузка плагинов из сомнительных источников может привести к печальным последствиям.\n" \
         "@fpc_plugins решает большинство возможных проблем.</b>"

au_user_settings = "Настройки для пользователя {}"
adv_fpc = "😎 FunPay Cardinal - лучший бот для FunPay"
...
🛠️ Создать своего бота: github.com/sidor0912/Funpay
💰 Донат: @sidor_donate
...
desc_lang = desc_main
desc_gs = "Здесь ты можешь включать и выключать основные функции <i>FPC</i>."
...
desc_pl = "Здесь ты можешь получить информацию о плагинах, а так же настроить их.\n\n" \
          "⚠️ <b><u>После активации / деактивации / добавления / удаления плагина, необходимо " \
          "перезапустить бота!</u></b> (/restart)"
desc_au = "Здесь ты можешь настроить авторизацию в панели управления Telegram."
desc_proxy = "Здесь ты можешь настроить прокси."
...
cmd_upload_chat_img = "(чат) выгрузить изображение на FunPay"
cmd_upload_offer_img = "(лот) выгрузить изображение на FunPay"
cmd_upload_plugin = "выгрузить плагин"
...
cmd_check_updates = "проверить на наличие обновлений"
cmd_update = "обновиться до след. версии"
...
v_edit_greeting_cooldown = "Введи кулдаун приветственного сообщения (в днях)."
...
v_photo = "<code>$photo=[PHOTO ID]</code> - фотография. Вместо <code>[PHOTO ID]</code> " \
          "впиши ID фотографии, полученный с помощью команды /upload_chat_img"
...
v_order_desc_and_params = "<code>$order_desc_and_params</code> - название заказа и/или параметры."
v_order_desc_or_params = "<code>$order_desc_or_params</code> - название заказа или параметры."
...
log_pl_delete_handler_err = "Произошла ошибка при выполнении хэндлера удаления плагина $YELLOW\"{}\"$RESET."
