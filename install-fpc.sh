...
echo -e "\n\n${RED} * GitHub ${CYAN}github.com/sidor0912/Funpay${RESET}"
...
#16
if ! sudo mkdir -p "/home/$username/Funpay"; then
  echo -e "${start_process_line}\nПроизошла ошибка при создании директории для бота. (16/${commands})\n${end_process_line}"
  exit 2
fi

#17
if ! sudo cp -r /home/$username/fpc-install/*/* /home/$username/Funpay/; then
...
#19
if ! sudo chown -hR $username:$username /home/$username/Funpay ; then
  echo -e "${start_process_line}\nПроизошла ошибка при изменении владельца файлов. (19/${commands})\n${end_process_line}"
  exit 2
fi

#20
if ! sudo -u $username /home/$username/pyvenv/bin/pip install -U -r /home/$username/Funpay/requirements.txt ; then
...
#21
if ! sudo ln -sf /home/$username/Funpay/Funpay@.service /etc/systemd/system/Funpay@.service ; then
...
clear
echo -e $logo
echo -e '\n\n\e[1;91m * GitHub \e[1;96mgithub.com/sidor0912/Funpay\e[0m'
...
CONFIG_FILE="/home/$username/Funpay/configs/_main.cfg"

if [ ! -f "$CONFIG_FILE" ]; then
    echo "Файл конфигурации не найден. Запускаем первичную настройку..."
    sudo -u $username LANG=en_US.utf8 /home/$username/pyvenv/bin/python /home/$username/Funpay/main.py <&1
else
    ...
    case "$edit_config" in
        [yY]|[yY][eE][sS])
            ...
            sudo -u $username LANG=en_US.utf8 /home/$username/pyvenv/bin/python -W ignore::SyntaxWarning /home/$username/Funpay/setup_telegram_proxy.py <&1
            ;;
        *)
...
sudo systemctl restart "Funpay@$username.service"
...
echo -e "\n\e[1;92mДля остановки FPC используй команду \e[93msudo systemctl stop Funpay@${username}\e[1;92m"
echo -e "Для запуска FPC используй команду \e[93msudo systemctl start Funpay@${username}\e[1;92m"
echo -e "Для перезапуска FPC используй команду \e[93msudo systemctl restart Funpay@${username}\e[1;92m"
echo -e "Для просмотра логов используй команду \e[93msudo systemctl status Funpay@${username} -n100\e[1;92m"
echo -e "Для добавления FPC в автозагрузку используй команду \e[93msudo systemctl enable Funpay@${username}\e[1;92m"
